...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      h4("Observations"),
      numericInput("time1", "Time after last dose (h)", 20),
      numericInput("dv1", "Concentration (mg/L)", 35),
      h4("Target"),
      numericInput("target", "Target concentration (mg/L)", 20.5),
      numericInput("ss", "Steady-state (days)", 30),
      h4("Perfom estimation"),
      actionButton("GO", "Individual MAP Estimation")
    ),
    mainPanel(
      tabsetPanel(
        tabPanel("Data",  tableOutput("mapbay_tab")),
        tabPanel("Concentrations",  plotOutput("concvstime")),
        tabPanel("ETA Distribution",  plotOutput("distparam")), 
        tabPanel("Alternative Regimens Results", 
...
server <- function(input, output) {
  ...
  output$concvstime <- renderPlot({
    mapbayr:::plot.mapbayests(my_est())+
      ggplot2::geom_hline(yintercept = input$target, linetype = 2) +
      ggplot2::geom_abline(intercept = 34, slope = 0, size=2, col="red",alpha=0.5,lty=2)+
      ggplot2::geom_abline(intercept = 20.5, slope = 0, size=2, col="green",alpha=0.5,lty=2)
    #We can also use :
    # shiny::req(my_est())
    # plot(my_est())
  })
